...
// Extended RTCIceServer interface to include credentialType
...
export class MediaMTXWebRTCReader {
  ...
  private static linkToIceServers(links: string | null): ExtendedRTCIceServer[] {
    return (links !== null) ? links.split(', ').map((link) => {
      ...
      const ret: ExtendedRTCIceServer = {
        urls: [m[1]],
      };
  ...
  private requestICEServers(): Promise<ExtendedRTCIceServer[]> {
  ...
  private setupPeerConnection(iceServers: ExtendedRTCIceServer[]): Promise<string> {
    if (this.state !== 'running') {
      throw new Error('closed');
    }
    ...
    return fetch(this.conf.url, {
      ...
      .then((res) => {
        switch (res.status) {
          ...
          case 400:
            return res.json().then((e: { error: string }) => { throw new Error(e.error); });
          default:
   ...
   */
  get connectionState(): ReaderState {
    return this.state;
  }
